...
def watch_video(filename: str):
    ...
    current_tags = _merge_video_tags(all_tags.get(filename, []))
...
def popular_tags_api():
    """Expose DB-backed popular tags for autocomplete."""
...
def video_tags_api():
    """Return current DB-backed tags for a single video."""
    ...
    all_tags = cache.get_tags()
    merged = _merge_video_tags(all_tags.get(filename, []))
...
def tags_page():
    """Tags page (canonical DB-backed read path)."""
    _prune_orphaned_tag_entries()
    tags = cache.get_all_unique_tags()
    return render_template('tags.html', tags=tags, tag_count=len(tags))


@app.route('/tag/<tag>')
@performance_monitor("route_tag_videos")
def tag_videos(tag):
    """Tag filtering using canonical DB-backed per-video tag map."""
    _prune_orphaned_tag_entries()
    normalized_tag = tag.lstrip("#")
    db_filtered_videos = cache.get_videos_by_tag_optimized(normalized_tag)
    filtered_videos = db_filtered_videos
