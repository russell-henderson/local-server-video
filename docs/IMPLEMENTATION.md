...
- `file_watcher.py` provides efficient, debounced monitoring with background thumbnail generation and duplicate detection.

### Architecture Highlights

- Modular Python scripts: `main.py`, `cache_manager.py`, `database_migration.py`, `performance_monitor.py`.
- Data files: `video_metadata.db` (SQLite), `favorites.json`, `ratings.json`, `tags.json`, `views.json`.
- Templates & static assets under `templates/` and `static/`.
...
Key implementation artifacts live in `static/` (e.g., `optimized-utils.js`, `video-preview-enhanced.js`, `favorites.js`) and `templates/_player.html`.
