...
function detectDevice() {
  // Pseudocode: inspect navigator, performance, and feature detection
  return {
    isMobile: /Mobile|Android|iP(hone|od|ad)/.test(navigator.userAgent),
    hasWebXR: !!(navigator.xr || navigator.xrSession),
...
function recoverFromPreviewError() {
  // Implement load-with-timeout, fallback handling, and cleanup
}
```
